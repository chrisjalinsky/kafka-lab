...
export JAVA_HOME={{ mapr_client_java_home }}
export MAPR_SUBNETS={{ mapr_cluster_subnets }}
export MAPR_HOME={{ mapr_loopbacknfs_home }}
    ...
    if [ -n "$JAVA_HOME" ]; then
        ...
        if [ $found -eq 1 ]; then
            if java_version=$($JAVA_HOME/bin/java -version 2>&1); then
                java_version=$(echo "$java_version" | fgrep version | head -n1 | cut -d '.' -f 2)
                if [ -z "$java_version" ] || [ $java_version -le 6 ]; then
                    unset JAVA_HOME
                fi
            else
                unset JAVA_HOME
...
if [ "$LIBPAM" != "" ]; then
    export LD_PRELOAD="$LD_PRELOAD $LIBPAM"
fi
